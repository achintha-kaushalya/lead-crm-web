...
function loadTodayStats() {
  ...
  statsEl.innerHTML = `
    <div class="stats-title">📊 Call Log (Today)</div>
    <div class="stat-row"><span class="stat-label">Total Calls</span><span class="stat-val blue">${total}</span></div>
    <div class="stat-row"><span class="stat-label">Contacted</span><span class="stat-val green">${contacted}</span></div>
    <div class="stat-row"><span class="stat-label">Converted</span><span class="stat-val yellow">${converted}</span></div>
    <div style="font-size:10px; opacity:0.5; margin-top:6px;">From local call logs</div>
  `;
}
